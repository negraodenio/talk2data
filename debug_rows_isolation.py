import os, pandas as pd, json
from supabase import create_client
from dotenv import load_dotenv

load_dotenv()
supabase = create_client(os.environ['SUPABASE_URL'], os.environ['SUPABASE_SERVICE_ROLE_KEY'])

def debug_rows():
    df = pd.read_excel("equities.xlsx")
    mapping = {
        'isin': 'ISIN', 'ticker': 'Ticker', 'name': 'Company Description',
        'currency': 'Currency', 'stock_price': 'Price', 'price_date': 'Price Date',
        'target_price': 'Target Price', 'target_price_date': 'Target Price Date',
        'dividend_yield': 'Dividend Yield', 'market_cap': 'Market Capitalization',
        'pe_ratio': 'Price to Earning Forward 12M', 'pe_forward_12m': 'Price to Earning Forward 12M',
        'sector_level1': 'Sector - Level 1', 'issuer_country': 'Issuer Country', 'region': 'Region'
    }

    print("🔍 Testando registros de 100 a 200 um por um...")
    for idx in range(100, 200):
        row = df.iloc[idx]
        rec = {}
        for db_col, excel_col in mapping.items():
            val = row[excel_col]
            if pd.isna(val) or str(val).strip() == "-": rec[db_col] = None
            else:
                if db_col in ['stock_price', 'target_price', 'dividend_yield', 'market_cap', 'pe_ratio', 'pe_forward_12m']:
                    try: rec[db_col] = float(str(val).replace('$','').replace('%','').replace(',','').strip())
                    except: rec[db_col] = None
                else: rec[db_col] = str(val).strip()
        
        try:
            supabase.table("equities").upsert(rec, on_conflict="ticker").execute()
            print(f"  ✓ {idx}: OK ({rec['ticker']})")
        except Exception as e:
            print(f"  ❌ {idx}: FALHOU ({rec['ticker']}) - {e}")
            print(f"     Payload: {json.dumps(rec, default=str)}")
            break # Parar no primeiro erro

if __name__ == "__main__":
    debug_rows()
